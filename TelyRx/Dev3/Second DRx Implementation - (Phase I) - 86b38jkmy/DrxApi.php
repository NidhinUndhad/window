...
class DrxApi extends AbstractHelper
{
    ...
    const COMMON_DRX_URL = '/external_api/v1/';
     ...
     * @return bool|string
     */
    public function getDrxApiKey($order, $storeId = null)
    {
        $apiMode = $this->getDrxApiMode();
        $mappedDrxData = $this->getMappedDrxData($order);
        if ($mappedDrxData) {
            if ($apiMode == "production") {
                return $mappedDrxData['drx_production_api_key'];
            } else {
                return $mappedDrxData['drx_demo_api_key'];
            }
        } else {
            if ($apiMode == "production") {
                return $this->scopeConfig->getValue(
     ...
     * @return bool|string
     */
    public function getEndpointsUrl($order, $storeId = null)
    {
        $apiMode = $this->getDrxApiMode();
        $mappedDrxData = $this->getMappedDrxData($order);
        ...
        if ($mappedDrxData) {
            if ($apiMode == "production") {
                return $mappedDrxData['drx_production_url'] . self::COMMON_DRX_URL;
            } else {
                return $mappedDrxData['drx_demo_url'] . self::COMMON_DRX_URL;
            }
        } else {
            if ($apiMode == "production") {
                return $this->scopeConfig->getValue(
        ...
        try {
            ...
            if (isset($customerData['telyrx_patient_id']) && !empty($customerData['telyrx_patient_id'])) {
                ...
                if (!empty($patientId) && is_numeric($patientId)) {
                    ...
                    $thirdParty = $this->createPatientThirdPartyInDrx($patientId, $order);
    ...
    public function createPatientInDrx($customerData, $order)
    {
        ...
        $endpointUrl = $this->getEndpointsUrl($order);
        ...
        $apiKey = $this->getDrxApiKey($order);
        $telephontArr = array();
    ...
    // public function getItemIdFromDrx($ndcNumber, $order)
    // {
    ...
    public function createPrescription($prescriptionData, $order)
    {
    ...
    public function createPrescriptionFromOrder($order)
    {
        ...
        try {
            ...
            foreach ($orderItems as $orderItemsKey => $orderItemsValue) {
                ...
                $preResult = $this->createOtcPrescription($prescriptionData, $order);
                        ...
                        if (array_key_exists('data', $dataResult)) {
                            if (array_key_exists(0, $dataResult['data'])) {
                                if (array_key_exists('rx_id', $dataResult['data'][0])) {
                                    ...
                                    if (count($result) > 0) {
                                        $this->getRefillRequest($postRefillRequest, $order_Id, $order);
                                    }
                                }
                            }
                        }
    ...
    public function prescription($drxPatiendId, $sigCode, $pillCount, $drxDoctorId, $skuNdc, $refillCount, $shippingMethod, $magentoCustomerId, $orderMain)
    {
        $days = date('Y-m-d H:m:s', strtotime('+364 days'));
        $customer = $this->customerFacory->create()->load($magentoCustomerId);
        $customerEmail = $customer->getEmail();
    ...
    public function skuNdc($product, $orderItemsValue)
    {
        ...
        if ($orderItemsValue->getProductType() == "configurable") {
            $simpleProductId = $orderItemsValue->getProductOptions()['info_buyRequest']['selected_configurable_option'];
    ...
    public function createOtcPrescription($prescriptionData, $order)
    {
        $result = array();
        $endpointUrl = $this->getEndpointsUrl($order);
        $prescriptionEndPt = $this->getCreatePrescriptionEndPoint();
    ...
    public function getRefillRequest($postRefillRequest, $order_Id, $order)
    {
     ...
     */
    public function createPatientThirdPartyInDrx($patientId, $order)
    {
        $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/prescription.log');
        $logger = new \Zend_Log();
        $logger->addWriter($writer);
        ...
        $endpointUrl = $this->getEndpointsUrl($order) . 'third-parties/' . $patientId;
        $apiKey = $this->getDrxApiKey($order);
    ...
    /**
     * Get mapped DRX data based on the shipping state of the order
     *
     * @param $order
     * @return array|null
     */
    public function getMappedDrxData($order)
    {
        try {
            if ($order->getShippingAddress()) {
                $shippingStateId = $order->getShippingAddress()->getRegionId();
                if ($shippingStateId) {
                    $connection = $this->_resource->getConnection();
                    $tableName = $this->_resource->getTableName('drx_instance_integration');

                    $query = $connection->select()
                        ->from($tableName, ['pharmacy_location', 'drx_production_url', 'drx_demo_url', 'drx_production_api_key', 'drx_demo_api_key'])
                        ->where('FIND_IN_SET(?, states_serviced)', $shippingStateId);
