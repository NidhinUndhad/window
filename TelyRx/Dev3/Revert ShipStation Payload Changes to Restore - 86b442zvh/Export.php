...
namespace Telyrx\Shipstation\Model\Action;

use Auctane\Api\Model\Action\Export as OriginalExport;
...
class Export extends OriginalExport
{
    /**
    ...
    private function writeOrderXml(Order $order): self
    {
        ...
        $this->addCustomFieldsToXml($order);
    ...
    /**
     * Write all custom fields into the XML response data.
     *
     * @param Order $order
     * @return void
     */
    private function addCustomFieldsToXml(Order $order)
    {
        if (!$order->getId()) {
            return;
        }
        ...
        if (!empty($customerFirstName)) {
            $this->addXmlElement("CustomField1", $customerFirstName);
        }
        if (!empty($customerLastName)) {
            $this->addXmlElement("CustomField2", $customerLastName);
        }
        if (!empty($pharmacyLocation)) {
            $this->addXmlElement("CustomField3", ucfirst($pharmacyLocation));
        }
    }
}
